...
- 너무 강결합 돼있기 때문에, 테스트 하기 쉬운 코드가 결합도가 낮다고 한다.
- 알고 있음에도, 어짜피 리펙토링을 할 것이므로, 짜보는 것! 

> class Sum implements Expression
- 다형성을 리마인드 하기 위해 적음

> 이 코드는 다음 두 가지 이유로 지저분하다. (p.124)
- 내 코드는 이러이러해서 지저분하다 라고 생각하지 않는데, 어느 부분이 문제인지 명시적으로 생각한 후에 단계적으로 고쳐 나가는 점이 인상적이었다

> 클래스를 명시적으로 검사하는 코드가 있을 때에는 항상 다형성(polymorphism)을 사용하도록 바꾸는 것이 좋다. (p.126)
- 다형성 습관이 잘 안되어있어서 리마인드하는 문장

> 우선 지금까지 한 작업을 검토해보자. (p.128)
- 각 단계에서 넘어가기전에 의식적으로 회고를 하는데,
- 이 부분이 나 스스로도 단계별로 일지를 쓰면 좋겠다 라는 생각을 했다
- 문제에 대해서, 이 문제를 이런게 문제라고 생각했고, 이렇게 고쳤다.
...
**ohahohah**      
- 신기하게 똑같은 내용인 것 같은데, 확 와닿는 부분이 다른게 굉장하다고 생각했다
