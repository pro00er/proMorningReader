...
## 20181109,20181112,20181113 - 부록.실전TDD - 네트워크 전까지
...
### 이야기
**ohahohah**     
> 인터페이스를 통해 할 수 있는 모든 일에 대해 명령줄 인터페이스로 응용 프로그램을 작성할 수 있어야 한다. (...) 모델 코드와 뷰의 코드가 완전히 분리되면 이 말은 바로 성립한다 . 
- 티디디 실습했을때 프린트하고 입력받는 부분은 테스트를 하지 않고 구조상으로 분리해버림. 판단이 잘 안설때 그때를 생각하면 인터페이스분리가훨씬쉬울듯. 어차피 분리되어있으면 CLI이든 다른 인터페이스 방식이든 인터페이스만 교체하면 되니 전체 구조에 영향을 미치지 않음
> 이는 테스트 가능성에서도 중요한데，TDD를 하면서 주기를 짧게 하기 위해 ‘할 수 있는 가장 단순한 일은 무엇인가? ’라고 질문하며 작성하다 보면 적절하게 유도된다 . TDD 에서의 리듬을 잘 유지하자 . (347)
- 리듬!
> 도메인 로직 코드를 확실하게 분리해 놓으면 재사용하기에도 훨씬 쉽고 테스트 가능성도 높아진다.
> 언제나 늘 모델부터 (...) 뷰를 위한 컨트롤들을 진열하기(...) 도메인 객체와 연결(...) 뷰_모델간의 상호작용
- 모델과 뷰, 비즈니스 로직 분리한다는게 와닿아요. 뷰-모델간 상호작용은 어차피 객체지향에서는 객체간의 메시지를 전달하는 거니까 그렇구나 싶고.
- ‘뷰는 가능한 한 얇게 만든다’가 잘 와닿지 않음
...
*   로그작성 & 리뷰
    *   로그 작성보다 정리하는데 품이 많이 드는거에 대해 부담을 느낌.
    *   로그 작성할 때 빠진 내용많아서 품이 많이 든다고 생각함.
    *   로그 올리고 리뷰하는게 품이 많이 드는 이유가 빠진 내용이 많아서 쓴 내용이 많아서 올리걸리는거 라고 생각함.
    *   저는 이슈를 올릴떄 체크해야할 부분이 너무 많아요. 이슈생성 - PR 날리이. 내용 편집에는 시간이 많이 걸리지 않는다. 다른 사람 내용 외에는 많이 수정하지 않는다. 의도를 왜곡할까봐. 마크다운형태로 만드는게 오래걸린다.
    *   kimsunoh : 품이 많이드는데 줄여야 되는게 많은데 줄여야되는게 뭔지 모름. Rudia님과 비슷하게 작업 줄임, 품을 줄이려면 키워드만 적고 문장에 맞지 않더라도 키워드만 적고 문장 고치는데 시간을 줄이자. 그 외에 건다필요하다고 생각 우리가 글을 남기는데 오탈자는 보기 편함인데 서로의 니즈라고 생각했고 키워드로 남기는 방법 찾자 한번에 적을때 키워드 중심으로 적어서 한번에 적어서  로그 올리는 사람의품을 줄이자. 3/1씩 똑같이 부담하지만
    *   2차 작업 외에 품을 줄이는 것
    *   이름만 수정하고 오탈자 눈에 거슬려도 상황 만 수정. 눈에 거술리는것 빼고 다 올리자.
    *   로그 작성은 내가 작성한 것 외에 다른 의도 있어서 타이핑 많아서 그걸 따라가지 못해 왜곡됨. 그 문장은 자기만 고칠 수있다.
    *   마크다운, 오탈자 검사기 하면 오탈자 수정됨. 마크다운은 좋은 앱이 있는데 타이포라 싹 붙여 놓은 다음에 일괄 수정은 타이프라이은으로 하고 툴을 사용하자.  다음 시간에 
    *   로그작성 길다
    *   키워드 작성하다가 문장
*   1주일치 내용 요약
    *   오: 퍼머넌트 링크의 개념으로 1주일치 내용을 정리. 이렇게 정리해놓으면 나중에 이 책을 치트시트처럼 사용
    *   문: 그럼 삼십분안에 안될 수도 있어요. 30분 이야기하고 30분 정리해야할 수도 있음
    *   김: 그냥 키워드 10개만 뽑아보면 안되나요?
    *   Action:  1주일치 키워드 요약 + 일일로그는 수정하지않고 master 에 바로 올림
*   실습만 나가자
    *   문: 세팅하자. 평일 아침 스터디시간을 투자해보자. 
    *   오: 1주일 정도 투자는 나쁘지 않다고 생각한다.  cloud9  다른데서도 쓸 수 있으니까
    *   김: 그렇게라도 해야할 거 같아요. 월요일부터 세팅해요.  
*   **[ ] 마틴 파울러 리팩토링 (다음주에 이야기)**
    *   > 다음주에 이야기하자
    *   ohahohah 시나리오에 맞게 하게 되어있음 비슷하게 해도 괜춘
    *   kimsunoh 실습을 했으면.. 자기 스스로 필요성 느끼고 찾아봐야한다. 처음엔 자기가 혼자 하기 힘드니깐.. 실습을 통해 도움ㅇ르 줄수있는데 더이상은 힘들지 않을까


### 회고

**ohahohah**
*   확실히 아침 시간으로 땡기는데 이번주가 처음인데 졸리고 난리남 컨디션 조절에 난항겪음 로그 품 줄여보자. 우리는 계속 개선해 나갈 거니깐

**kimsunoh**
*   ohahohah님 마지막 문장 멋있구요. 우형이 개발자 뽑는다는 페이스북 글을 봤는데 개발자는 쓸모있는 걸 만드는 사람이라는 거라더구요. 만드는 사람, 성장하는 사람 이라는 뜻이 있는데 우리는 그걸 하고 있다. 여러분 덕분에 우리는 개발자 답게 하고 있구나 생각이 들었다.
*   몸이 나았다. 돌아왔다. 

**RudiaMoon**
*   이 정리할 수 있는 시간이 의미가 있다. 개선해나가고


## 20180718 - 26장. 빨간 막대 패턴
...
> 아는 것에서 모르는 것으로(knowntounknown)' 라는 방향이 유용할 것이다
...
> **한 단계 테스트**  구현할 수 있다는 확신이 드는 테 스트를 고를 것(p.217)



*   용기는 불확실성에 대한 것 같다.

> 내가 테스트 목록을 볼 땐 보통 이런 식이다. "이건 뻔하지, 이것도 뻔 하고, 이건 잘 모르겠군，이건 뻔하고，이건… 내가 무슨 생각으로 적은 거 지? 아，이건 할 수 있겠다."이 마지막 테스트가 바로 내가 다음으로 구현 할 테스트인 것이다. **뻔하진 않지만 구현할 수 있다는 확신이 있다**(p.218)



*   뻔하진 않지만, 구현할 수있는 확신이 드는것. 키워드

>아는 것에서 모르는 것으로(knowntounknown)' 라는 방향이 유용할 것이다. '아는 것에서 모르는 것으로' 는 우리가 어느 정도의 지식과 경험을 가지고 개발을 시작한다는 점，개발 하는 중에 새로운 것을 배우게 될 것임을 예상한다는 점 등을 암시한다. (p.219)



*   작은 단위로 쪼개야하나 이런 원칙이 아니라 아는거에서 모른는걸로 라는게 형식이 줄어든다. 
*   우리는 저희는 안다고 착각한 것에서 모르는것으로 시작한 경향이 있다.
*   아는 것에서 익숙한 거에서 해보고 점점 조금씩 모르는 케이스로 넘어가
...
*   아침에 일어났는데 너무 더워서 슬펐어요.
*   잠들었다가 행아웃소리에 깼어요.
*   30분 더 일찍했다고 이렇게 힘들수가
*   TDD 루즈해져요.
...
*   **미리 깬 상태에서 읽으니까 좋네요.**
*   **앞으로 테스트실습하게 될텐데 여기서 좋은 건 쏙쏙 뽑아서 해보면 재밌을 것같아요.**
...
> 다음 테스트를 고를 때 무엇을 기준으로 할 것인가? 여러분에게 새로운 무언가를 가르쳐 줄 수 있으며, 구현할 수 있다는 확신이 드는 테스트를 고를 것.

각 테스트는 여러분을 최종 목표로 한 단계 진전 시켜 줄 수 있어야 한다.(p.217)



*   kimsunoh : 테스트를 고를 때 무슨 기준으로 고를까?가 전 문장에 나오는데 테스트에는 계속 켄트백아저씨가 말하는데 할려는 마음 생기는 나는 할 수 없지만 넌 할 수 있을거야 라고 하는데 확신을 주는 테스트하는게 중요. 테스트를 만드는게 목적이아니고 테스트 기능을 구현하는게 목적. 똑같은 테스트를 만들필요 없다. 테스트 중복이 없어지는 그런게 이 문장안에 있다.

>한 단계 테스트는 시작 테스트에도 적용된다. 당신에게 뭔가를 가르쳐 줄 수 있으면서도 빠르게 구현할 수 있는 테스트를 선택하라. 만약 당신이 어떤 애플리케이션을 n번째 구현하고 있다면，오퍼레이션을 한두 개 필요로 하는 테스트를 하나 골라라. 당신은 그걸 작동하게 할 수 있을 거라 자신할 것이다. 만약 뭔가 어렵고 복잡한 것을 처음 구현해보고 있다면 당신은 즉각 조그만 용기의 알약을 하나 먹어야 한다(p.220)



*   RudiaMoonMoon : 읽다가 시간이 다된것을 알아서 끝에서부터 인상적인걸 가져왔다.
*   우리가 보통 시작 테스트 할 때 순서를 정해논 대로 하는 것 같다. 이 책에서 말하는대로 설계한 후, 빠른 테스트가 가능한 것부터 해보자.우리 실습에선 이 부분이 빠진 것 같다. 
*   ohahohah : 한단계씩 하는 거라는게, 
*   RudiaMoonMoon : 저는 쉬운 것 부터 한단계씩에 초점을 맞추고 싶다 (ohahohah : 아하!)
*   kimsunoh : 

> 어떻게 해야 하나? 단순한 시작법은 테스트를 이용하여 묻고，테스트를 이용하여 설명하는 것이다(p.221)
...
*   앞으로 지각 안할 거예요. ohahohah님 출첵소리에 깼다.

**RudiaMoon**



*   비몽사몽하니까 정신이 없고. 정신이 없어요...
...
*   RudiaMoonMoon :  수식을 적어놓으면 더 편하다고 한다. 이것도 주석이 필요할까? 했는데, 이것을 결과값이 아니라 assert문으로 나타내주더라, 그런데 assert문이 아니면 어떻게 표현하지? 했다. 다른 사람들은 어떻게 생각하는지 물어보고 싶다
*   ohahohah : 함수로 구현할 것 같아요
*   kimsunoh : assert문을 만드는 이유가 있을텐데 A길이 있는데 A길을 가기위해 다른 방법을 가는게 의미가 있을까.
*   ohahohah : 이게 최종코드를 말하는거 같진 않다. 중복이 있어서 마음에 안든다. 
*   assert new note(49.25)익스펙티드한 값이 아닌 과정을 다 써줘라 이게 더 나으려나? 이런 생각이 든다. 컨버트 함수 구현전에 어떻게 써야할지 모르니깐 명백한 상수를 넣어준다. 
*   리팩토링 안 나오는 것이다.라는 걸까요? 
*   컨버트하는 역할이 수식 결과 값을 리턴하는것
*   Rudia: 위에 코드와 아래 코드를 비교하면 아래가 더 보기 쉬운 거 같아요. 근데 상수를 많이 쓰면 힘든데. 테스트케이스라 이렇게 쓰는건가.
*   kimsunoh: 매직넘버를 쓰지말라는 예외적인 규칙이 테스트코드에서는 좋은게 아닐까? 
...
*   ohahohah : 스코프개념 이해해야해요/ pulblic private 스코프가 statice도 스코프인데 이미 정해진 기호상수가 있다면 이라는 부분이 테스트 케이스 정의된 넒은 스코프가 있다면 기호상수를 쓰겠다 이건데 이 테스트 케이스 안에 쓰게되면 무의미하다.
*   kimsunoh : 스코프에 대한 개념 다시 한번 넘어가죠

> 하지만 이미 정의된 기호 상수가 있다면 나 는 그것을 사용할 것이다.
...
*   이 책이 한번 그렇구나 이랬는데 '아~ 이런거였어?' 계속 읽어도 뭔가가 있는 그런 책이라 상당히 재미있고 이 책 쉬운책 아니라고 했는데 이걸 이해하려면 다른것도 이해해야하고 그런 이해 범위가 커지면 이해 범위가 커지는 범위라 재미있는 책이다.
*   업무일지를 리뷰하는 형식이래요. 스터디가 생각나서 시간 오래 걸리지 않다. 오타 이런거 다 그대로 두고 퍼머넌트 도큐먼트는 맥락만 전달 할 수 있게. 우린 일이 너무 많은 거 같다. 이런 얘기가 많은데 그렇다면 일지는 러프하게 왜곡되지 않게 그대로 올리고 토요일날 퍼머넌트 문서를 만드는건 어떨까. 도큐먼트가 핵심이 아닌데 (kimsunoh - 핵심이 되어 가고 있다. 약간 뭔가 책 읽는것 보다 로그 작성하는일이 커지는 일.)
*   이 로그 자체가 그런 힘을 갖지 못하는 거면, 힘을 빼고 가는게 어떨까? 라는 생각을 했다.
*   얘기 해 볼 내용 : ?로그 작성 힘 빼는 것?
...
*   이 책 그래도 빨리 안나가고 세세하게 해서 좋았음. 짧은 문장도 길게 이야기해볼수 있고. 좋았다.
