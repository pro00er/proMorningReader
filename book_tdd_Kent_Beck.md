...
•테스트를 확실히 돌아가게 만드는 세 가지 접근법 : 가짜로 구현하기，
삼각측량법, 명백하게 구현하기.
•설계를 주도하기 위한 방법으로 테스트 코드와 실제 코드 사이의 중
복을 제거하기.
...
**ohahohah**     
>  다음에 할 일은 무엇인가 I
- 이 챕터가 전반적으로 다 좋았다. 한번 다시 되돌아보는 점이 좋았다
- 이 책의 접근 방법이 좋았다
> 내가 지금까지 작성한 모든 프로그램을 20번씩 다시 작성해본다면 어떨
까? 매번 새로운 통찰을 얻고 놀라움을 경험할 것인가? 더 신중을 기해서 모든 통찰을 처음 세 번 안에 다 얻어낼 방법이 있을까? 아니면 한번에?

> JUnit 사용도
- 코드 메트릭스, Juit사용도를 수치적으로 보여주면서, 정량적으로 평가하는 방법을 보는 것이 좋았다. TDD를 전체적으로 사용하지 않는 회사에 있다면, 개인이 회고하는 것엔 한계가 있기 때문에 이런 수치적인 데이터가 보이는게 좋았다
jit, 네이버에서 쪽에서 쓰는 무언가? 혼자 할때 이런거 필요.
...
- 생각할만해. 견고한다라는부분에서 테스트 필요해.
> 나는 작업을 끝낸 후에 스몰토크의 스몰린트(SmallL加 ) 같은 코드 감정 (code critic) 프로그램을 실행하길 좋아한다 (p.150)
- 코드리뷰부분 같이 하면 재미있울 것 같다
> *다음에 할일은 무엇인가T 에 관련된 또 다른 질문은 “어떤 테스트들이 추가로 더 필요할까?1’다. 때로는 실패해야 하는 테스트가 성공하는 경우가 있는데,그럴 땐 그 이유를 찾아내야 한다 (p.150)
- 뭘만들어야할지 생각하고 바로 코드들어가는 프로그래밍하는걸 먼저들어가는데 기능적인  a->a 나와야함
> 할일 목록이 빌 때가 그때까지 설계한 것을 검토하기에 적절한 시기다. (p.150)
- 할일없으면 설계검토를 하는 것에  공감했다. 현재 설계하고선 코드분석하고있는 업무 맡고 있어서, 설계하면서 노는게아니라 검토가 필요하다는 거에 (뜨끔)했다.
...
**ohahohah**
- 책 마무리해서 좋았음. 삼각측량법 한장한장 읽을땐 아는 내용인데 알지 못하고있었음. 내가 안다고 착각하고있음. 착각한다라는 걸 안다는게 좋았음. 내가 모르는걸 깨달을때 좋았어요.
뿌듯했다.     
- ksno의 말에 공감함
...
**Rudia**      
- 서식을 바꿨다. 시영님이 리드를하면 나를 제일 먼저 시킨다는 생각이 들어서 이다. 하지만, 그게 아니라는 것을 알게 되었다.
