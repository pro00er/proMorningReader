...
## 20181020 
## 이야기
- 각자 담당 pr가져가기로 한거 진행사항 공유 - 오늘까지 하자  
  - [ ] ohah : 실습  
  - [ ] rudia:회의,책(이번주)  
- [ ] 쌓이는 로그 깃헙에 어떻게 적을까? 
  예쩐처럼 매일매일 단위로?    
  하루에 한번씩 올릴때는 로그 올리면서 리마인드하는 기분이라 좋았다.
- [ ] 뇌풀기 퀴즈? -> 스터디 본격시작 5분전(07:25)에 준비시간 공지
...
http://techquiz.itworld.co.kr/solver/quiz_list
그러면 일어났음 신고하고 각자 머리를 풀 수 있을만한 준비시간을 가진다 
- [ ] 다음주 진행은 어떻게? 실습
저희가 온라인 실습하다가 잘 안되었는데 왜 안되었는지 이야기해보자. 
전 일단 세팅 가정이 좀 많이 힘들었다고 생각해요 그때 진행자가 그 행아웃 켜 있었어요 근데 이번에 간단하다고 생각한 건 팀뷰어만 안 되니까
턴하려다보니 아침 30분으로는 시간이 부족 
20분에 세팅하고 30분부터 하자 였는데 잘안되었다. 
월수금 하고 1시간 하자
왜 30분이 부족했나? 
모두 아침시간이라 머리가 몽롱. 그래서 로직을 짤때 버퍼링이 많이 걸렸다. 
매일 하는 실습을 하는게 아니라 월요일 다음에 수요일 이렇게 텀이 있었고, 텀때문에 저번시간에 뭐했지? 하고 기억을 더듬는 과정이 오래걸림.
  - 온라인의 특성상 페어의 단점  
뭔가 의사소통이 A를 생각하면서 이야기하고있는데 A-1이야를 생각하면서 바로바로 이해못함. 직접 만나면 그림을 그리던가 등등 다른 걸로 서로 이해시키는 방법을 여러가지를 사용했을텐데 거의 목소리로만 피드백을 하다보니 이렇게 맞춰가는게 오래 걸렸다. 시간은 한정되어있는데. 그래서 온라인 페어 프로그래밍이 어렵다.라고 느낌. 
...
현업이나 실제에서 티디디를 쓰지 않는데 실습시간 안에 채워줄 순 없다. 실습시간에는 ‘TDD를 써봤다’라는걸 초점으로 두지 말고 평소에 내가 티디디를 쓰고 있는 방식을 ‘피드백 받는 시간’이라고 생각했으면 좋겠다.
다음주차 해야할 것
TDD실습을 하는게 맞는지 잘 모르겠어요. 
시간투자가 계속 걸리고있다. 이 스터디시간에 TDD실습을 하는게 맞는걸까 배우는걸 활용하는것도 좋은데 그거는 현업에서 다른시간에 충분히 할 수 있다고 생각한다. 코드리뷰를 받는다던가 이런게 맞는 방법일까? 이런 생각이 든다.  개인 프로젝트를 하고나서 같이 나눌 수도 있고 이런부분 막히고 있어서 리뷰해줘 이런 방법이 있는데 이 시간안에 실습을 하는게 늘어지고있다라는 걸 개선사항은 없는 건데 그럼에도 불구하고 실습을 해야한다는 건 잘 모르겠다. 
저자의 글 나눔을 하고 생각하기
TDD실습을 하는 것
전 위에 적은 장점이 있기 때문에 tdd 실습을 하고 싶어요 최대 실수를 아까부터 하고 싶었다고 했기 때문에 저자의 글을 저자의 글을 하고 나서 티비를 하는게 어떨까 생각을 했던 거고요
TDD 실습을 해보되 예전문제 내가 배운게 얼마나 더 발전했는지 되돌아 볼 수있는 점에서 예전에는 내가 우리가 이런 식으로 썻었는데 이번에 다시 풀어 보니까 전보다 좀 더 빠른 시간 안에 끝낼 수 있었고 그렇기 때문에 내가 전 보다는 조금 더 또 다른 방식으로 생각했던 거를 보면서 진도 한 달은 또 다른 혈관 느낄 것 같아요 근데 새로운 문제를 하게 되면 새로운 문제를 어떻게 풀어 자기야 될지 또 생각을 하게되고 그러다 보면은 다시 우주의지고 뭐 이럴 수 있을 거 같아서 한 번 더 풀어 보는게 좋을 것 같아요 
책 읽기
- [ ] 김창준 아티클 공유 (시영)  
- [ ] sunoh 컴백 일정  
...
시간이 길어지면 온라인에서도 잠깐 쉬고 이야기하는 방법이 필요하다. 논쟁해야할 필요가 없다고 생각한다. 그러면서 생각한게 오늘 피알도 하루에 한번씩 날리자 라는 주제도 안했던 이유가 있는거잖아요. 그러면 내가 이거에 대해서 예전에는 이렇게 안되었었고 보완해야하는 방법이 뭐였는지 이야기해봐야했나 장단점을 이야기해보는 거. 온라인 실습이랑 피알하는 거 둘다 장단점을 이야기해봐야하지 않았나?
다른 방법이 무엇일지에 대해서 이야기해보는 게 좋을 거 같다
